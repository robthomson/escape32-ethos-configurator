...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build an ETHOS Suite-installable ESCape32 Lua package zip."
    )
    parser.add_argument(
        "--artifact-version",
        required=True,
    ...
    parser.add_argument(
        "--artifact-name",
        help="Output zip filename. Defaults to escape32-<version>.zip.",
    )
    parser.add_argument(
        "--manifest-version",
    ...
    parser.add_argument(
        "--package-name",
        default="ESCape32 Lua Ethos Tool",
        help="Display name written into ethos_lua_manifest.json.",
    )
    parser.add_argument(
        "--package-key",
        default="org.escape32.ethos-tool",
    ...
    parser.add_argument(
        "--package-dir",
        default=DEFAULT_PACKAGE_DIR,
        help="Directory under src/ to package into the manifest install folder.",
    )
    parser.add_argument(
        "--build-root",
    ...
    parser.add_argument(
        "--keep-build-root",
        action="store_true",
        help="Keep the scratch directory instead of deleting it after packaging.",
    )
    parser.add_argument(
        "--output-dir",
        default=".",
...
def main() -> int:
    ...
    artifact_name = args.artifact_name or f"escape32-{args.artifact_version}.zip"
    zip_path = output_dir / artifact_name

    using_temp_build_root = args.build_root is None
    build_root = (
        Path(tempfile.mkdtemp(prefix="escape32-package-", dir=str(output_dir))).resolve()
        if using_temp_build_root
        else Path(args.build_root).resolve()
    )
    package_root = build_root / "package"
    source_root = SRC_ROOT / args.package_dir
    staged_folder = package_root / args.folder

    if not source_root.is_dir():
        raise FileNotFoundError(f"Package source directory not found: {source_root}")

    base_version = read_base_version(source_root / "ui.lua")
    ...
    try:
        ...
        shutil.copytree(source_root, staged_folder, dirs_exist_ok=True)
        update_staged_main_version(staged_folder / "ui.lua", args.artifact_version)
        files = collect_files(package_root)
        manifest_files = [f"{args.folder}/**"]

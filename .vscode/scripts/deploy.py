...
def _load_connect_module():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("am32_connect", CONNECT_MODULE_PATH)
...
def _stage_mkdir(prefix='am32-stage-'):
...
def copy_files(src_override, fileext, targets, lang="en", steps=None):
    ...
    def _stage_tree(src_dir: str):
        stage_root = _stage_mkdir(prefix="am32-stage-")
...
def main():
    ...
    p.add_argument('--lang', default=os.environ.get("AM32_LANG", "en"),
                   help='Locale to resolve if deploy steps use one. Defaults to env AM32_LANG or "en".')
